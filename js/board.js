...
const Board = (() => {
  ...
  const HEX_R    = 28;
  const HEX_W    = Math.sqrt(3) * HEX_R;   // ~48.5  (center-to-center horizontal)
  const HEX_H    = 2 * HEX_R;              // 56     (center-to-center vertical = 1.5*R)
  const SHEAR    = Math.tan(30 * Math.PI / 180);   // tan(30°) ≈ 0.577
  const SCALE_Y  = 0.75;
  ...
  function _corner(col, row) {
    const rx = col * HEX_W + (row % 2 === 0 ? 0 : HEX_W / 2);
    const ry = row * HEX_R * 1.5;
